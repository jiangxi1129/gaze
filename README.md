...
> 让你接入的 AI（Claude / ChatGPT / 任何能调外部工具的）实时"看到"你电脑屏幕，靠便宜的视觉模型生成短弹幕描述 + OCR 抓字幕，写到一个 AI 在对话开场时能读到的"记忆 store"上。
...
**是**：
- 一个跑在你本地 Windows 电脑上的 Python 脚本，定时截屏 → 截图压一个 perceptual hash 去重 → 没变化跳过 → 有变化送一次截图给视觉模型（智谱 GLM-4V-Flash 之类）拿一句弹幕描述 + OCR 抓屏上文字 → 推给某个 AI 能读到的"记忆"服务
- 配套有：浮窗显示当前状态、音频字幕（可选）、窗口黑名单（隐私）、自动跟随前台窗口、长期记忆归档
